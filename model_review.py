...
def inference(user_input: str, model: AutoPeftModelForCausalLM, tokenizer: AutoTokenizer, device: str) -> str:
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate structured academic paper reviews using a trained language model.")
    parser.add_argument("--pdf_file", type=str, required=True, help="Path to the PDF file.")
    parser.add_argument("--model_id", type=str, default="travis0103/mistral_7b_paper_review_lora", help="Hugging Face model identifier.")
    parser.add_argument("--quantize", action="store_true", help="Apply quantization to the model for performance optimization.")
    ...
    if "cuda" in args.device and not torch.cuda.is_available():
        raise RuntimeError("CUDA is not available. Please switch to CPU or install a GPU.")
    if args.device == "cpu" and args.quantize:
        raise RuntimeError("Quantization is not supported on CPU. Please switch to a CUDA device.")
